...
public class SyncService extends IntentService {
	...
	private void callListWebServices() {
		...
		try {
			...
			for (MSPSSyncMenu mspsSyncMenu : sm) {
				...
				if (!callIsOk) {
					m_IsRunning = callIsOk;
					//	2015-12-06 Dixon Martinez
					//	Change break for continue
					continue;
					//	End Dixon Martinezs
				}
			}
			...
			if(m_LastMsg == null) {
				...
				Env.setIsEnvLoad(Env.getCtx(),true); //red1 -- to let isEnvLoad be true during Login right away
			} else  {
				sendStatus(SyncValues.BC_STATUS_END,
	...
	private boolean callWebService(StringNamePair... Params){
		...
		if (so != null){
			...
			so = null;
			il = null;
			if (CurrentWS < CountWS){ //CurrentWS - CountWS != 0
				if (CurrentPage - pages != 0 && pages > 0){
					...
					//	Set to Call
					sendStatus("CurrentPage -  "+CurrentPage+" ("+pages+") "+ CurrentWS+" of "+CountWS, false, -1);
					...
					//	Set to Call
					sendStatus("CURRENT of TOTAL - "+CurrentWS+" of "+CountWS, false, -1);
					return callWebService(p_Params);
				}
			}
			else
			{
				if (CurrentPage < pages && pages > 0) { //CurrentPage - pages != 0
