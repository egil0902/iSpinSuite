...
public class MSPSSyncMenu extends X_SPS_SyncMenu{
	...
	public static List<MSPSSyncMenu> getNodesFromParent(Context ctx,String p_ParentNode, DB conn) {
		...
		sql= "SELECT treend.Parent_ID, " + // 0
			...
			"WHERE tab.TableName = ? AND treend.Parent_ID = ? AND sm.IsActive ='Y'" +
			"ORDER By treend.SeqNo ";
		
		try{
			
			rs = conn.querySQL(sql, new String[]{MSPSSyncMenu.Table_Name, p_ParentNode});
			...
			if(rs.moveToFirst()){
	    		do{
					...
					if (rs.getString(3).equals("Y")){
						items.add(item);
						items.addAll(getNodesFromParent(ctx,rs.getString(1),conn));
	    			}
					else
						items.add(item);
